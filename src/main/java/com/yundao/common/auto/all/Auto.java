...
public class Auto {
	...
	public void create(String driver,String url,String user,String passwd,String tableName,String userPackageName) throws Exception{
		...
		CreateModel model = new CreateModel(colClassList,className,classPackageName,parentPackageName,pk,dbpk);
		model.createModelJava(util);//创建model实体类
		model.createMysqlSqlmapXML(util);//创建mysql sqlmap配置文件
//
		CreateDao dao = new CreateDao(colClassList,className,classPackageName,parentPackageName,pk,dbpk);
		dao.createIDAOJava(util);//创建DAO接口类
		
		CreateService service = new CreateService(colClassList, className, classPackageName, parentPackageName, pk, dbpk);
		service.createServiceJava(util);//创建service接口类
		service.createServiceImplJava(util);//创建service实现类
//
//
		CreateController controller = new CreateController(colClassList, className, classPackageName, parentPackageName, pk, dbpk);
