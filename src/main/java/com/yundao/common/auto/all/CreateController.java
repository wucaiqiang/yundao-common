...
public class CreateController {
	...
	public CreateController(List<String> colClassList,String className,String classPackageName,String parentPackageName,String pk,String dbpk) {
	...
	public void createControllerJava(Util util) throws Exception {
		...
		fileStrList.add("package " + classPackageName+".controller."+util.toLower(className)+";");
		...
		fileStrList.add("import "+util.getUserPackageName()+".dto."+util.toLower(className)+"."+classRsqDto+";");
...
//		fileStrList.add("import com.yundao.core.utils.ObjectCopyUtil;");
		...
		fileStrList.add("@RequestMapping(value = \"/"+util.toLower(className)+"/\")");
		...
		if(!pk.equals("")){
			...
			fileStrList.add("		return " + util.toLowerFrist(classNameService) + ".selectPage(model, pageDto);");
			fileStrList.add("    }");
			fileStrList.add("");
			...
			fileStrList.add("    public Result<Integer> add(@Validated @ModelAttribute "+classRsqDto+" dto, BindingResult bindingResult) throws Exception{");
...
//			fileStrList.add("        "+classNameModel+" model = ObjectCopyUtil.copyObject(dto,"+classNameModel+".class);");
			fileStrList.add("        return "+util.toLowerFrist(classNameService)+".insert(model);");
			fileStrList.add("    }");
			fileStrList.add("");
			...
			fileStrList.add("    public Result<Integer> update(@Validated(value = {Update.class}) @ModelAttribute "+classRsqDto+" dto, BindingResult bindingResult) throws Exception{");
			fileStrList.add("        BindingResultHandler.handleByException(bindingResult);");
			fileStrList.add("        "+classNameModel+" model = new " + classNameModel + "();");
			fileStrList.add("        BeanUtils.copyProperties(dto,model);");
			fileStrList.add("        return "+util.toLowerFrist(classNameService)+".update(model);");
			fileStrList.add("    }");
			fileStrList.add("");
			...
			fileStrList.add("       return "+util.toLowerFrist(classNameService)+".select(" + pk +");");
			fileStrList.add("    }");
			fileStrList.add("");
		}
		...
		fileStrList.add("}");
		util.writeFile(fileStrList, "controller/"+util.toLower(className)+"/", classNameController+".java");
	}
}
